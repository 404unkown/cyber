...
// Function to add reaction to a command message FROM OTHERS ONLY
async function addCommandReaction(sock, message) {
    try {
        // Check if message is from the bot itself - if yes, skip reaction
        ...
        if (!userMessage.startsWith('.')) return; // Only react to commands
        
        // Add a small delay to make it more natural
        await new Promise(resolve => setTimeout(resolve, 500));
        
        // Get a random emoji from the expanded list
...
async function handleAreactCommand(sock, chatId, message, isOwner) {
    try {
        ...
        if (action === 'on') {
            ...
            await sock.sendMessage(chatId, { 
                text: '✅ Auto-reactions have been enabled globally\n\nI will now react with random emojis when others use commands!',
                quoted: message
            });
        } else if (action === 'off') {
