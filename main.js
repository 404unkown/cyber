...
const xminusCommand = require('./commands/xminus');
...
const { sswebCommand, sswebMobileCommand } = require('./commands/ssweb');
const attpCommand = require('./commands/attp');
const aiStudioCommand = require('./commands/aistudio');
...
const editfotoCommand = require('./commands/editfoto');
...
const { dmblockerCommand, readState: readdmblockerState } = require('./commands/dmblocker');
const settingsCommand = require('./commands/settings');
const soraCommand = require('./commands/sora');
const ttvCommand = require('./commands/ttv');
...
async function handleMessages(sock, messageUpdate, printLog) {
    try {
        ...
        const ownerCommands = ['.mode', '.autostatus', '.antidelete', '.cleartmp', '.setpp', '.clearsession', '.areact', '.autoreact', '.autotyping', '.autoread', '.dmblocker'];
            ...
            case userMessage.startsWith('.close'):
                {
                    ...
                    if (closeArg !== undefined && (isNaN(closeDuration) || closeDuration <= 0)) {
                        await sock.sendMessage(chatId, { text: 'Please provide a valid number of minutes or use .close with no number to close immediately.', ...channelInfo }, { quoted: message });
                    } else {
                        await closeCommand(sock, chatId, senderId, message, closeDuration);
                    }
                }
                break;
            case userMessage === '.open':
                await openCommand(sock, chatId, senderId);
                break;
    ...
    await ttsCommand(sock, chatId, message, ttsText);
    commandExecuted = true;
    break;
            ...
            case userMessage.startsWith('.anticall'):
                ...
                break;
            case userMessage.startsWith('.dmblocker'):
                {
                    const args = userMessage.split(' ').slice(1).join(' ');
                    await dmblockerCommand(sock, chatId, message, args);
                }
                commandExecuted = true;
                break;
...
case userMessage.startsWith('.editfoto') || userMessage.startsWith('.nano'):
    const editfotoArgs = userMessage.split(' ').slice(1);
    await editfotoCommand(sock, chatId, message, editfotoArgs);
    commandExecuted = true;
    break;
...
case userMessage.startsWith('.aifigure'):
case userMessage.startsWith('.aicomic'):
case userMessage.startsWith('.aiwm'):
    const aiCommand = userMessage.split(' ')[0].slice(1);
    const aiArgs = userMessage.split(' ').slice(1);
    await aiStudioCommand(sock, chatId, message, aiCommand, aiArgs);
    commandExecuted = true;
    break;
...
case userMessage.startsWith('.ssweb'):
    const sswebUrl = userMessage.split(' ').slice(1).join(' ');
    await sswebCommand(sock, chatId, message, [sswebUrl]);
    commandExecuted = true;
    break;
case userMessage.startsWith('.ttv'):
case userMessage.startsWith('.t2v'):
case userMessage.startsWith('.gen'):
    const ttvArgs = userMessage.split(' ').slice(1);
    await ttvCommand(sock, chatId, message, ttvArgs);
    commandExecuted = true;
    break;

case userMessage.startsWith('.sswebm') || userMessage.startsWith('.ssmobile'):
    const ssmobileUrl = userMessage.split(' ').slice(1).join(' ');
    await sswebMobileCommand(sock, chatId, message, [ssmobileUrl]);
    commandExecuted = true;
    break;
case userMessage.startsWith('.wormgpt') || userMessage.startsWith('.worm'):
    const wormgptQuery = userMessage.split(' ').slice(1).join(' ');
    await wormgptCommand(sock, chatId, message, [wormgptQuery]);
    commandExecuted = true;
    break;
...
case userMessage.startsWith('.veo3'):
    ...
    const veo3Prompt = userMessage.split(' ').slice(1).join(' ');
    await veo3Command(sock, chatId, message, [veo3Prompt]);
    commandExecuted = true;
    break;
            case userMessage.startsWith('.remini') || userMessage.startsWith('.enhance') || userMessage.startsWith('.upscale'):
                await reminiCommand(sock, chatId, message, userMessage.split(' ').slice(1));
                break;
case userMessage.startsWith('.xminus'):
case userMessage.startsWith('.vocalcut'):
case userMessage.startsWith('.separate'):
    const xminusArgs = userMessage.split(' ').slice(1);
    await xminusCommand(sock, chatId, message, xminusArgs);
    commandExecuted = true;
    break;
           case userMessage.startsWith('.sora'):
case userMessage.startsWith('.imagetovideo'):
case userMessage.startsWith('.i2v'):
case userMessage.startsWith('.img2vid'):
    const soraArgs = userMessage.split(' ').slice(1);
    await soraCommand(sock, chatId, message, soraArgs);
    commandExecuted = true;
    break;
            default:
                if (isGroup) {
