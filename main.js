...
const autogroupCommand = require('./commands/autogroup');
const footballCommand = require('./commands/football');
const { autorecordingCommand, handleAutorecordingForMessage, handleAutorecordingForCommand } = require('./commands/autorecording');
...
async function handleMessages(sock, messageUpdate, printLog) {
    try {
        ...
        if (!userMessage.startsWith('.')) {
            ...
            // ⭐ Auto-recording for regular messages (works like autotyping)
...
if (!isGroup) {
    await handleAutoreply(sock, chatId, senderId, userMessage, message);
        ...
        const ownerCommands = ['.mode', '.autostatus', '.antidelete', '.cleartmp', '.setpp', '.clearsession', '.areact', '.autoreact', '.autotyping', '.autoread', '.pmblocker', '.autorecording'];
...
case userMessage === '.creator':
    ...
    break;
case userMessage.startsWith('.football'):
    await footballCommand(sock, chatId, message, userMessage.split(' ').slice(1));
    commandExecuted = true;
    break;
case userMessage.startsWith('.autogroup'):
    await autogroupCommand(sock, chatId, message, userMessage.split(' ').slice(1));
    commandExecuted = true;
    break;
    case userMessage.startsWith('.autorecording'):
    const recordingArgs = userMessage.split(' ').slice(1);
    await autorecordingCommand(sock, chatId, message, recordingArgs);
    commandExecuted = true;
    break;
case userMessage.startsWith('.autoreply'):
    ...
    // 1. A voice note with caption .autoreply
    // 2. A text reply to a voice note with .autoreply voice on
    ...
    if (isVoiceNote || isReplyingToVoiceNote) {
        console.log('🎤 Voice note scenario detected');
        console.log('🎤 Is direct voice note?', isVoiceNote);
        console.log('🎤 Is reply to voice note?', isReplyingToVoiceNote);
        
        // Check if command contains "voice on" when replying
        const isVoiceOnCommand = userMessage.includes('voice on');
        
        // Create enhanced message with all info
        const enhancedMessage = {
            ...message,
            _isVoiceNote: isVoiceNote,
            _isReplyToVoiceNote: isReplyingToVoiceNote,
            _originalCommand: userMessage,
            _isVoiceOnCommand: isVoiceOnCommand
        };
        
        await autoreplyCommand(sock, chatId, enhancedMessage);
    } else {
...
case userMessage === '.vcf':
    ...
    commandExecuted = true;
    break;
case userMessage.startsWith('.imgscan'):
case userMessage.startsWith('.scanimg'):
case userMessage.startsWith('.imagescan'):
case userMessage.startsWith('.analyzeimg'):
    ...
    break;
        ...
        if (commandExecuted !== false) {
           ...
           await handleAutorecordingForCommand(sock, chatId, message);
        }
